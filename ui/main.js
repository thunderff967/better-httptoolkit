...
`,vb=G.div`
    ...
    word-break: break-word;
    white-space: pre-wrap;
`;function bb(e){return!!e.language}const xb=e=>(t,n)=>le(Kl(t,e,n)),dbg=window.dbg=(...args)=>{try{if(window.htkUtilsDebug!==false)console.debug('[Utils]',...args)}catch(e){}},decodeProtobuf=window.decodeProtobuf=(buffer)=>{
    let offset = 0;
    const fields = {};
    const readVarint = () => {
        let value = 0;
    ...
    while (offset < buffer.length) {
        ...
        const tagNum = header >> 3;
        let fieldVal;
        if (wireType === 0) {
            fieldVal = { wireType: 0, value: readVarint() };
        } else if (wireType === 1) {
            if (offset + 8 > buffer.length) break;
            const slice = buffer.slice(offset, offset + 8);
            offset += 8;
            fieldVal = { wireType: 1, value: Array.from(slice, b => b.toString(16).padStart(2, '0')).join('') };
        } else if (wireType === 2) {
...
let isText = false;
            try {
                const str = new TextDecoder('utf-8', { fatal: true }).decode(subBuf);
                if (str.length > 0) {
                    isText = true;
                    for (let i = 0; i < str.length; i++) {
                        const code = str.charCodeAt(i);
                        if (code < 32 && code !== 9 && code !== 10 && code !== 13) { isText = false; break; }
                    }
                }
            } catch (e) {}
            if (!isText) {
                try {
                    const sub = decodeProtobuf(subBuf);
                    if (Object.keys(sub).length > 0) {
                        fieldVal = { wireType: 2, isMessage: true, value: sub, rawBytes: subBuf };
                    }
                } catch (e) {}
            }
            if (!fieldVal) {
                if (isText) {
                    fieldVal = { wireType: 2, isString: true, value: new TextDecoder().decode(subBuf), rawBytes: subBuf };
                } else {
                    let binary = "";
                    for (let i = 0; i < subBuf.length; i += 8192) {
                        binary += String.fromCharCode.apply(null, subBuf.subarray(i, i + 8192));
                    }
                    fieldVal = { wireType: 2, isBytes: true, value: btoa(binary), rawBytes: subBuf };
                }
            }
        } else if (wireType === 5) {
            if (offset + 4 > buffer.length) break;
            const val = new DataView(buffer.buffer, buffer.byteOffset + offset, 4).getInt32(0, true);
            offset += 4;
            fieldVal = { wireType: 5, value: val };
        } else {
            break;
        }
        if (!fields[tagNum]) {
    ...
    return fields;
},parseProtoSchema=window.parseProtoSchema=(schemaText)=>{
    const messages = {};
    if (!schemaText) { dbg('parseProtoSchema: empty schema text'); return messages; }
    const cleanSchema = schemaText.replace(/\/\/.*$/gm, '').replace(/\/\*[\s\S]*?\*\//g, '');
    const messageRegex = /message\s+(\w+)\s*\{([\s\S]*?)\}/g;
    let match;
    while ((match = messageRegex.exec(cleanSchema)) !== null) {
        const msgName = match[1];
        const msgBody = match[2];
        const fields = {};
        const fieldRegex = /^\s*(repeated|optional|required)?\s*([\w\.]+)\s+(\w+)\s*=\s*(\d+)\s*;/gm;
        let fieldMatch;
        while ((fieldMatch = fieldRegex.exec(msgBody)) !== null) {
    ...
    dbg('parseProtoSchema: messages=', Object.keys(messages));
    return messages;
},decodeMessage=window.decodeMessage=(buffer,typeName,schemas)=>{
    const fields = decodeProtobuf(buffer);
    const schema = schemas[typeName];
    dbg('decodeMessage:', typeName, 'bytes=', buffer.length, 'rawFields=', JSON.stringify(fields).slice(0, 500), 'schema=', schema ? JSON.stringify(schema) : null);
    if (!schema) {
        const result = {};
        for (const tag in fields) {
            const val = fields[tag];
            if (Array.isArray(val)) {
                result[tag] = val.map(v => v.value);
            } else {
    ...
    for (const tag in fields) {
        const val = fields[tag];
        const fieldDef = schema[tag];
        const key = fieldDef ? fieldDef.name : tag;
        const valMapper = (v) => {
            if (fieldDef && schemas[fieldDef.type] && v.rawBytes) {
                return decodeMessage(v.rawBytes, fieldDef.type, schemas);
            }
            return v.value;
        };
        if (Array.isArray(val)) {
            result[key] = val.map(valMapper);
        } else {
            result[key] = valMapper(val);
        }
    }
    dbg('decodeMessage result:', typeName, JSON.stringify(result).slice(0, 500));
    return result;
},runDecryptionPipeline=window.runDecryptionPipeline=async(buffer,pipeline)=>{
    if (!(Array.isArray(buffer) || buffer instanceof ArrayBuffer || ArrayBuffer.isView(buffer) || (buffer && typeof buffer.byteLength === 'number'))) {
        throw new Error('Expected a byte buffer (ArrayBuffer, Uint8Array, or Array) for decryption');
    }
let data = new Uint8Array(buffer);
    dbg('runDecryptionPipeline: input bytes=', data.length, 'steps=', pipeline.map(s => s.type));
    let prevLen = data.length;
    for (const step of pipeline) {
        dbg('Step start:', step.type, 'input bytes=', prevLen, step.type === 'aes_decrypt' ? 'keyLen=' + (step.key || '').length + ' ivLen=' + (step.iv || '').length : '', step.type === 'protobuf_parse' ? 'messageName=' + (step.messageName || '') + ' schemaChars=' + (step.schema || '').length : '');
        try {
        if (step.type === 'base64_decode') {
        ...
        } else if (step.type === 'hex_decode') {
            const str = new TextDecoder().decode(data).trim();
            const match = str.match(/[a-fA-F0-9]{2}/g) || [];
            data = new Uint8Array(match.map(h => parseInt(h, 16)));
} else if (step.type === 'url_decode') {
            const str = new TextDecoder().decode(data);
            const bytes = [];
            for (let i = 0; i < str.length; i++) {
                if (str[i] === '%' && i + 2 < str.length) {
                    const hex = parseInt(str.substr(i + 1, 2), 16);
                    if (!isNaN(hex)) {
                        bytes.push(hex);
                        i += 2;
                        continue;
        ...
        } else if (step.type === 'aes_decrypt') {
            const keyBytes = new TextEncoder().encode(step.key || '');
            const ivBytes = new TextEncoder().encode(step.iv || '');
            let paddedKey = new Uint8Array(16);
            if (keyBytes.length > 16) paddedKey = new Uint8Array(32);
            paddedKey.set(keyBytes.subarray(0, paddedKey.length));
            let paddedIv = new Uint8Array(16);
            paddedIv.set(ivBytes.subarray(0, 16));
            ...
            data = new Uint8Array(decryptedBuf);
        } else if (step.type === 'protobuf_parse') {
            if (step.schema && step.messageName) {
                const schemas = parseProtoSchema(step.schema);
                const decodedJson = decodeMessage(data, step.messageName, schemas);
                const jsonStr = JSON.stringify(decodedJson, null, 2);
                data = new TextEncoder().encode(jsonStr);
            } else {
                const decodedProto = decodeProtobuf(data);
                const jsonStr = JSON.stringify(decodedProto, null, 2);
                data = new TextEncoder().encode(jsonStr);
            }
}
            dbg('Step end:', step.type, 'output bytes=', data.length);
        } catch (err) {
            dbg('Step FAILED:', step.type, err, err && err.stack);
            throw err;
        }
        prevLen = data.length;
    }
    const final = new TextDecoder().decode(data);
    dbg('runDecryptionPipeline: done, output bytes=', data.length, 'output=', final);
    return final;
},UtilsPage=window.UtilsPage=() => {
    const [rules, setRules] = b.useState(() => {
        try {
            return JSON.parse(localStorage.getItem('htk-autodec-rules') || '[]');
        } catch (e) { return []; }
    });
    const [editingRule, setEditingRule] = b.useState(null);
    const [name, setName] = b.useState('');
    const [matchUrl, setMatchUrl] = b.useState('');
    const [matchMethod, setMatchMethod] = b.useState('ANY');
    const [applyTo, setApplyTo] = b.useState('both');
    const [pipeline, setPipeline] = b.useState([]);
    const [newStepType, setNewStepType] = b.useState('aes_decrypt');

    const saveRules = (newRules) => {
        setRules(newRules);
        localStorage.setItem('htk-autodec-rules', JSON.stringify(newRules));
        window.activeDecryptionRules = {};
    };
    const handleAdd = () => {
        setEditingRule('new');
        setName('New Decryption Rule');
        setMatchUrl('');
        setMatchMethod('ANY');
        setApplyTo('both');
        setPipeline([]);
    ...
    const handleEdit = (rule) => {
        setEditingRule(rule);
        setName(rule.name || '');
        setMatchUrl(rule.matchUrl || '');
        setMatchMethod(rule.matchMethod || 'ANY');
        setApplyTo(rule.applyTo || 'both');
    ...
    const handleSave = () => {
        if (!matchUrl) {
            alert('Please enter a host or URL pattern to match.');
            return;
        }
        const newRule = {
            id: editingRule === 'new' ? Math.random().toString(36).slice(2) : editingRule.id,
            name, matchUrl, matchMethod, applyTo, pipeline
        };
        if (editingRule === 'new') {
            saveRules([...rules, newRule]);
        } else {
            saveRules(rules.map(r => r.id === newRule.id ? newRule : r));
        }
        setEditingRule(null);
    };

    const addPipelineStep = () => {
        const newStep = { id: Math.random().toString(36).slice(2), type: newStepType };
        if (newStepType === 'aes_decrypt') {
            newStep.key = '';
            newStep.iv = '';
        } else if (newStepType === 'protobuf_parse') {
            newStep.schema = '';
            newStep.messageName = '';
        }
    ...
    const updateStepField = (id, field, val) => {
        setPipeline(pipeline.map(s => s.id === id ? Object.assign(Object.assign({}, s), { [field]: val }) : s));
    };
    ...
    return b.createElement("div", {
        ...
        b.createElement("h1", { style: { fontSize: "28px", fontWeight: "300", marginBottom: "8px", color: "#fff" } }, "Utils"),
        b.createElement("div", { style: { display: "flex", gap: "16px", borderBottom: "1px solid #333", marginBottom: "20px" } },
        ...
        editingRule ? b.createElement("div", {
            style: {
            ...
            b.createElement("h3", { style: { margin: "0 0 10px 0", color: "#fff" } }, editingRule === 'new' ? "Add Auto-Decryption Rule" : "Edit Auto-Decryption Rule"),
            b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "4px" } },
                b.createElement("span", { style: { fontSize: "12px", color: "#aaa" } }, "Rule Name"),
                b.createElement("input", {
                    type: "text",
                    value: name,
                    onChange: ev => setName(ev.target.value),
                    style: { padding: "8px", backgroundColor: "#2b2b2b", color: "white", border: "1px solid #444", borderRadius: "4px" }
                })
            ),
            b.createElement("div", { style: { display: "flex", gap: "10px" } },
                b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "4px", flex: 1 } },
                    b.createElement("span", { style: { fontSize: "12px", color: "#aaa" } }, "Match Method"),
                    b.createElement("select", {
                        value: matchMethod,
                        onChange: ev => setMatchMethod(ev.target.value),
                        style: { padding: "8px", backgroundColor: "#2b2b2b", color: "white", border: "1px solid #444", borderRadius: "4px" }
                    },
                        b.createElement("option", { value: "ANY" }, "ANY"),
                        b.createElement("option", { value: "GET" }, "GET"),
                        b.createElement("option", { value: "POST" }, "POST"),
                        b.createElement("option", { value: "PUT" }, "PUT"),
                        b.createElement("option", { value: "DELETE" }, "DELETE")
                    )
                ),
                b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "4px", flex: 1.2 } },
                    b.createElement("span", { style: { fontSize: "12px", color: "#aaa" } }, "Apply Decryption to"),
                    b.createElement("select", {
                        value: applyTo,
                        onChange: ev => setApplyTo(ev.target.value),
                        style: { padding: "8px", backgroundColor: "#2b2b2b", color: "white", border: "1px solid #444", borderRadius: "4px" }
                    },
                        b.createElement("option", { value: "both" }, "Request & Response"),
                        b.createElement("option", { value: "request" }, "Request Only"),
                        b.createElement("option", { value: "response" }, "Response Only")
                    )
                ),
                b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "4px", flex: 2 } },
                    b.createElement("span", { style: { fontSize: "12px", color: "#aaa" } }, "Match Host / URL Substring"),
                    b.createElement("input", {
                        type: "text",
                        placeholder: "e.g. api.game.com or login",
                        value: matchUrl,
            ...
            b.createElement("div", { style: { borderTop: "1px solid #333", paddingTop: "12px", marginTop: "4px" } },
                b.createElement("span", { style: { fontSize: "14px", fontWeight: "bold", color: "#fff" } }, "Decryption & Decoding Pipeline"),
                b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "8px", marginTop: "8px" } },
                    pipeline.map((step, idx) => b.createElement("div", {
                        ...
                        b.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center" } },
                            b.createElement("span", { style: { fontWeight: "bold", fontSize: "12px", color: "#0275d8" } }, "Step " + (idx + 1) + ": " + step.type.toUpperCase().split("_").join(" ")),
                            b.createElement("button", {
                                onClick: () => removePipelineStep(step.id),
                                style: { backgroundColor: "transparent", color: "#d9534f", border: "none", cursor: "pointer", fontSize: "12px" }
                            }, "Remove")
                        ),
                        step.type === 'aes_decrypt' && b.createElement("div", { style: { display: "flex", gap: "10px" } },
                            b.createElement("input", {
                                ...
                                style: { flex: 1, padding: "6px 8px", backgroundColor: "#1e1e1e", color: "white", border: "1px solid #444", borderRadius: "3px", fontSize: "12px" }
                            }),
                            b.createElement("input", {
                                type: "text",
                                placeholder: "AES IV",
                        ...
                        step.type === 'protobuf_parse' && b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "6px" } },
                            b.createElement("input", {
                                type: "text",
                                ...
                                style: { padding: "6px 8px", backgroundColor: "#1e1e1e", color: "white", border: "1px solid #444", borderRadius: "3px", fontSize: "12px" }
                            }),
                            b.createElement("textarea", {
                                ...
                                style: { padding: "6px 8px", backgroundColor: "#1e1e1e", color: "white", border: "1px solid #444", borderRadius: "3px", fontSize: "12px", minHeight: "80px", fontFamily: "monospace", resize: "vertical" }
                            })
                        )
                    ))
                ),
                b.createElement("div", { style: { display: "flex", gap: "10px", marginTop: "12px", alignItems: "center" } },
                    b.createElement("select", {
                        ...
                        style: { padding: "6px", backgroundColor: "#2b2b2b", color: "white", border: "1px solid #444", borderRadius: "4px", fontSize: "12px" }
                    },
            ...
            b.createElement("div", { style: { display: "flex", gap: "10px", marginTop: "12px", justifyContent: "flex-end" } },
                b.createElement("button", {
                ...
                b.createElement("button", {
                    onClick: handleSave,
                    style: { padding: "8px 16px", backgroundColor: "#0275d8", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }
                }, "Save")
            )
        ) : b.createElement("div", { style: { maxWidth: "800px" } },
            b.createElement("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" } },
                b.createElement("span", { style: { color: "#aaa", fontSize: "14px" } }, "Configure rules to automatically decrypt and format matching traffic in the View tab."),
                b.createElement("button", {
                    onClick: handleAdd,
                    style: { padding: "8px 16px", backgroundColor: "#0275d8", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "bold" }
                }, "+ Add Rule")
            ),
            rules.length === 0 ? b.createElement("div", {
                style: {
                    padding: "40px",
                    textAlign: "center",
                    backgroundColor: "#1e1e1e",
            ...
            }, "No auto-decryption rules configured yet.") : b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "12px" } },
                rules.map(rule => b.createElement("div", {
                    key: rule.id,
                    style: {
                        padding: "16px",
                        backgroundColor: "#1e1e1e",
                    ...
                    b.createElement("div", { style: { display: "flex", flexDirection: "column", gap: "4px" } },
                        b.createElement("div", { style: { fontWeight: "bold", color: "#fff", fontSize: "15px" } }, rule.name),
                        b.createElement("div", { style: { fontSize: "13px", color: "#aaa" } },
                            b.createElement("span", { style: { backgroundColor: "#333", padding: "2px 6px", borderRadius: "3px", marginRight: "6px", color: "#0275d8", fontWeight: "bold" } }, rule.matchMethod),
                            b.createElement("span", { style: { backgroundColor: "#444", padding: "2px 6px", borderRadius: "3px", marginRight: "6px", color: "#aaa", fontSize: "11px" } }, (rule.applyTo || "both").toUpperCase()),
                            "matches URL containing: ",
                            b.createElement("code", { style: { color: "#00bc8c", fontFamily: "monospace" } }, rule.matchUrl)
                        ),
                        b.createElement("div", { style: { fontSize: "12px", color: "#777", marginTop: "2px" } },
                            "Pipeline Chain: " + (rule.pipeline && rule.pipeline.length > 0 ? rule.pipeline.map(s => s.type.replace("_", " ")).join(" ➔ ") : "None")
                        )
                    ),
                    b.createElement("div", { style: { display: "flex", gap: "8px" } },
                        b.createElement("button", {
                            onClick: () => handleEdit(rule),
                            style: { padding: "6px 12px", backgroundColor: "#333", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "13px" }
                        }, "Edit"),
                        b.createElement("button", {
                            onClick: () => handleDelete(rule.id),
                            style: { padding: "6px 12px", backgroundColor: "#d9534f", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer", fontSize: "13px" }
                        }, "Delete")
                    )
                ))
            )
